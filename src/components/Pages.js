  ...
  return (
    <>
      {sections.map((section, index) => (
        <div className="col-4 col-12-medium" key={`Column${index}`}>
          {section}
        </div>
      ))}
    </>
...
const Style1 = ({
  ...
  <section
    id={node.id}
    key={node.id}
    className={`wrapper style${style} bottom inactive`}
  >
    <Fade bottom big>
      <div className="content">
        <div className="container">
          <div className="row">
            ...
            {!gallery && !location && <Columns>{RichText({ content: node.body })}</Columns>}
          </div>
        </div>
      </div>
      <ScrollLink
...
const processNodes = (nodes) => {
  ...
  return displayNodes
    .map((node, index) => ({
      ...node,
      nextSection:
        (displayNodes[index + 1] && displayNodes[index + 1].id) ||
        "last-section",
    }))
    .map((node, index) => {
      let {
      ...
      let style = slug === "/" || !featuredImage ? 1 : (index % 2) + 2;
...
const Pages = ({ nodes }) => <>{processNodes(nodes)}</>;
